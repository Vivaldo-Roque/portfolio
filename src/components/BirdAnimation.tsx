...
export const BirdAnimation: React.FC = () => {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        const meshColor = Math.random() * 0xffffff;
        const birdGeometry = new (Bird as any)();
        const birdMaterial = new THREE.MeshBasicMaterial({ color: meshColor, side: THREE.DoubleSide });
      ...
      const render = () => {
        for (let i = 0; i < birds.length; i++) {
          ...
          let brightness = (500 - bird.position.z) / 1000;
  ...
  return <div id="bird-container" ref={containerRef} style={{ width: '100%', height: '100%', position: 'absolute', top: 0, left: 0, zIndex: 0 }} />;
};
